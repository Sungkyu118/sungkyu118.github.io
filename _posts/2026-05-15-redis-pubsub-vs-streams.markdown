...
Pub/Sub와 Streams는 둘 다 이벤트를 흘려보내는 것처럼 보여도, 실제로는 메시지 보관과 재처리 관점에서 성격이 꽤 다릅니다. 그래서 [Redis for 큐: List로 시작하고 Streams를 고민하는 기준](/redis/queue), [Redis Streams 실전: 작업 큐로 쓰는 법](/redis/streams-queue), [Redis 자료구조 2: List와 Set, 순서와 중복을 다루는 법](/redis/ds-lists-sets) 글을 함께 보시면 좋겠습니다. 비교 글만 따로 보는 것보다, 큐 설계와 실제 Streams 사용 글을 같이 보셔야 어떤 상황에서 무엇을 선택해야 하는지 더 덜 헷갈리게 정리됩니다.
...
- [Redis for 큐: List로 시작하고 Streams를 고민하는 기준](/redis/queue)
  ...
  Pub/Sub와 Streams를 비교하기 전에 큐 요구사항을 먼저 이해하면 선택 기준이 훨씬 명확해집니다. 큐 글을 함께 읽으면 메시지를 한 번만 처리해야 하는지, 실패 시 다시 처리해야 하는지, 순서가 얼마나 중요한지 같은 질문을 더 구체적으로 세울 수 있습니다.

- [Redis Streams 실전: 작업 큐로 쓰는 법](/redis/streams-queue)

  Streams는 Pub/Sub와 달리 메시지 보관과 소비자 그룹을 다룰 수 있어 작업 큐에 더 가까운 흐름을 만들 수 있습니다. Streams 실전 글을 함께 보면 비교표로만 보던 차이가 실제 명령과 처리 흐름에서 어떻게 드러나는지 확인할 수 있습니다.

- [Redis 자료구조 2: List와 Set, 순서와 중복을 다루는 법](/redis/ds-lists-sets)

  메시지 처리도 결국 순서, 중복, 보관 여부라는 자료구조 감각과 이어집니다. List와 Set 글을 함께 읽으면 이벤트 전달 방식을 고를 때 단순히 기능 이름만 비교하지 않고 데이터의 성질부터 차분히 생각할 수 있습니다.
