...
실시간 랭킹은 예시로 보기에는 재미있지만, 실제 구현에서는 정렬 구조 선택과 운영 비용을 같이 보지 않으면 금방 단순 데모에 머무르게 됩니다. 그래서 [Redis 자료구조 3: Sorted Set(ZSET), 랭킹의 정석](/redis/ds-zset), [Redis 입문 실무형 3: 운영 기본, 메모리와 eviction을 먼저 보자](/redis/practical-ops-basics), [Redis 운영 심화: 메모리, eviction, 핫키 사고 패턴](/redis/memory-eviction-hotkeys) 글도 함께 읽어보시면 좋겠습니다. 이렇게 같이 보시면 랭킹 구현 자체뿐 아니라, 인기 키가 몰렸을 때 어떤 문제가 생길 수 있는지도 더 현실적으로 보이실 것입니다.
...
- [Redis 자료구조 3: Sorted Set(ZSET), 랭킹의 정석](/redis/ds-zset)
  ...
  랭킹 기능을 제대로 이해하려면 ZSET의 점수와 정렬 원리를 먼저 자세히 보는 것이 좋습니다. 이 글을 함께 읽으면 랭킹 예제 코드가 왜 ZADD, ZREVRANGE, ZRANK 같은 명령으로 구성되는지 더 자연스럽게 연결됩니다.

- [Redis 입문 실무형 3: 운영 기본, 메모리와 eviction을 먼저 보자](/redis/practical-ops-basics)

  랭킹은 사용자 눈에 바로 보이는 기능이라 운영 중 지연이나 데이터 누락이 체감되기 쉽습니다. 운영 기본 글을 같이 읽으면 메모리 사용량, 모니터링, 장애 대응 관점에서 랭킹 데이터를 어떻게 바라봐야 하는지 더 현실적으로 준비할 수 있습니다.

- [Redis 운영 심화: 메모리, eviction, 핫키 사고 패턴](/redis/memory-eviction-hotkeys)

  인기 랭킹은 특정 키에 요청이 몰릴 가능성이 크기 때문에 핫키와 eviction 문제를 꼭 함께 생각해야 합니다. 운영 심화 글을 읽으면 기능 구현 자체보다 더 어려운 병목, 메모리 정책, 트래픽 집중 문제를 미리 떠올릴 수 있습니다.
