...
Streams를 작업 큐로 쓰는 방식은 단순 예제만 보면 좋아 보이지만, 실제로는 왜 List가 아니라 Streams를 택했는지부터 분명해야 판단이 흔들리지 않습니다. 그래서 [Redis for 큐: List로 시작하고 Streams를 고민하는 기준](/redis/queue), [Redis Pub/Sub vs Streams: 이벤트 전달을 어디에 써야 할까](/redis/pubsub-vs-streams), [Redis 자료구조 2: List와 Set, 순서와 중복을 다루는 법](/redis/ds-lists-sets) 글을 같이 읽어보시면 좋겠습니다. 이 흐름으로 보시면 Streams가 단순 상위 호환이 아니라, 요구사항이 달라질 때 선택하는 다른 도구라는 점이 더 또렷하게 정리됩니다.
...
- [Redis for 큐: List로 시작하고 Streams를 고민하는 기준](/redis/queue)
  ...
  Streams를 보기 전에 List 기반 큐의 장점과 한계를 먼저 이해하면 왜 더 복잡한 구조가 필요한지 자연스럽게 납득할 수 있습니다. 큐 글을 함께 읽으면 단순한 작업 처리에서 시작해 재처리, 소비자 그룹, 메시지 추적이 필요한 순간까지 단계적으로 이어집니다.

- [Redis Pub/Sub vs Streams: 이벤트 전달을 어디에 써야 할까](/redis/pubsub-vs-streams)

  Streams는 Pub/Sub와 자주 비교되기 때문에 두 방식의 차이를 분명히 알고 있어야 선택이 흔들리지 않습니다. 비교 글을 함께 읽으면 실시간 브로드캐스트와 보관 가능한 작업 흐름이 어떤 요구사항에서 갈라지는지 더 차분하게 정리할 수 있습니다.

- [Redis 자료구조 2: List와 Set, 순서와 중복을 다루는 법](/redis/ds-lists-sets)

  Streams는 별도의 자료구조처럼 느껴지지만, 순서와 중복을 어떻게 다룰지 고민한다는 점에서는 List와 Set의 기본 감각과도 이어집니다. 이 자료구조 글을 함께 보면 큐를 설계할 때 데이터의 순서, 소비 여부, 중복 가능성을 먼저 따져보는 습관을 만들 수 있습니다.
