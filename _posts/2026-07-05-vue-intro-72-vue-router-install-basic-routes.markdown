---
layout: post
title: "Vue 입문 72: Vue Router 설치와 기본 라우트 구성하기"
date: 2026-07-05 07:12:00 +0900
category: Vue
permalink: /vue/intro-72-vue-router-install-basic-routes
description: "Vue Router를 설치하고 createRouter, createWebHistory, routes 배열, app.use(router) 흐름으로 기본 라우트를 구성하는 방법을 설명합니다."
image:
  path: "/assets/img/og/vue-series-cover.svg"
  alt: "Vue 입문 시리즈 대표 이미지"
tags: [Vue, Vue Router, createRouter, 라우트 설정]
---

# Vue 입문 72: Vue Router 설치와 기본 라우트 구성하기

> Vue Router의 기본 설치 흐름과 앱에 router 플러그인을 등록하는 순서를 익힙니다.
>
> 이전 글: [Vue 입문 71: SPA에서 라우터가 필요한 이유](/vue/intro-71-why-spa-needs-router)
> 다음 글: [Vue 입문 73: router-view와 router-link가 하는 일](/vue/intro-73-router-view-router-link)
> 함께 보면 좋은 글:
...
Vue Router를 쓰려면 먼저 라우터 인스턴스를 만들고 Vue 앱에 등록해야 합니다. 처음 보면 `createRouter`, `createWebHistory`, `routes`, `app.use(router)`가 한꺼번에 등장해서 낯설 수 있습니다. 하지만 역할을 나누어 보면 생각보다 단순합니다.

`routes`는 어떤 주소에서 어떤 컴포넌트를 보여줄지 적는 표입니다. `createRouter`는 그 표와 히스토리 방식을 받아 라우터 객체를 만듭니다. 마지막으로 `app.use(router)`를 호출하면 Vue 앱 전체에서 `RouterLink`, `RouterView`, `useRoute`, `useRouter`를 사용할 수 있게 됩니다.

중요한 순서는 `mount` 전에 router를 등록해야 한다는 점입니다. 앱을 이미 화면에 붙인 뒤에 플러그인을 등록하면 라우터가 초기 URL을 제대로 해석하지 못하거나, 라우터 관련 기능이 기대대로 동작하지 않을 수 있습니다.
...
아래 예제는 Vite 기반 Vue 프로젝트에서 가장 기본적인 라우터 구성을 만드는 흐름입니다.
...
`routes` 배열의 각 객체가 하나의 페이지 규칙입니다. `path`는 주소이고, `component`는 그 주소에서 보여줄 컴포넌트입니다. `name`은 나중에 문자열 경로 대신 이름으로 이동할 때 유용합니다. `createWebHistory()`는 일반적인 깨끗한 URL을 사용하게 해주지만, 운영 환경에서는 서버 fallback 설정이 필요하다는 점도 기억해야 합니다.
...
- **확인 포인트**: `app.use(router)`는 `mount` 전에 호출해야 합니다. 순서가 바뀌면 라우터가 앱 초기화 과정에 제대로 참여하지 못합니다. 입문 예제에서는 작게 보일 수 있지만, 실제 프로젝트에서는 원인을 찾기 어려운 버그나 사용자 경험 문제로 이어질 수 있습니다. 코드를 작성한 뒤에는 이 문제가 어떤 상황에서 나타날 수 있는지 직접 한 번 재현해보는 습관을 들이면 좋습니다.
- **확인 포인트**: 경로 alias인 `@`를 쓰려면 Vite 설정에서 alias가 잡혀 있어야 합니다. 설정이 없다면 상대 경로로 import하거나 alias 설정을 확인해야 합니다. 입문 예제에서는 작게 보일 수 있지만, 실제 프로젝트에서는 원인을 찾기 어려운 버그나 사용자 경험 문제로 이어질 수 있습니다. 코드를 작성한 뒤에는 이 문제가 어떤 상황에서 나타날 수 있는지 직접 한 번 재현해보는 습관을 들이면 좋습니다.
- **확인 포인트**: `createWebHistory()`를 쓰면 운영 서버가 모든 프론트 경로를 `index.html`로 돌려주는 설정이 필요합니다. 이 설정이 없으면 직접 URL 접근 시 404가 날 수 있습니다. 입문 예제에서는 작게 보일 수 있지만, 실제 프로젝트에서는 원인을 찾기 어려운 버그나 사용자 경험 문제로 이어질 수 있습니다. 코드를 작성한 뒤에는 이 문제가 어떤 상황에서 나타날 수 있는지 직접 한 번 재현해보는 습관을 들이면 좋습니다.
...
처음에는 한 파일에 routes를 모두 적어도 괜찮습니다. 하지만 페이지가 늘어나면 도메인별 route 파일로 분리하는 것이 좋습니다. 너무 일찍 분리하면 구조가 과해지고, 너무 늦게 분리하면 라우터 파일이 거대한 목차처럼 커집니다.
...
이번 글에서는 'Vue 입문 72: Vue Router 설치와 기본 라우트 구성하기' 주제를 중심으로 Vue 프로젝트에서 자주 만나는 구조 문제를 살펴봤습니다. 핵심은 코드를 예쁘게 나누는 것이 아니라, 상태와 책임이 어디에 있는지 설명할 수 있게 만드는 것입니다. 예제 코드를 그대로 따라 작성한 뒤에는 이름을 바꿔보거나, 실패 상황을 일부러 만들어보거나, 조건을 하나 추가해보면서 흐름을 직접 확인해보시면 좋습니다.
