---
layout: post
title: "Vue 입문 63: useFetch를 만들며 로딩, 성공, 실패 상태 다루기"
date: 2026-07-05 07:03:00 +0900
category: Vue
permalink: /vue/intro-63-use-fetch-loading-error
description: "Vue Composable로 useFetch를 만들며 API 요청의 data, loading, error 상태를 분리하고 실패 상황까지 다루는 방법을 설명합니다."
image:
  path: "/assets/img/og/vue-series-cover.svg"
  alt: "Vue 입문 시리즈 대표 이미지"
tags: [Vue, Composable, useFetch, API]
---

# Vue 입문 63: useFetch를 만들며 로딩, 성공, 실패 상태 다루기

> API 호출을 화면에 직접 흩뿌리지 않고 loading, data, error 상태로 나누어 관리하는 법을 배웁니다.
>
> 이전 글: [Vue 입문 62: useCounter로 가장 작은 Composable 만들어보기](/vue/intro-62-use-counter-composable)
> 다음 글: [Vue 입문 64: Composable 안에서 watch를 사용할 때 주의할 점](/vue/intro-64-watch-inside-composable-cautions)
> 함께 보면 좋은 글:
...
실제 화면을 만들기 시작하면 거의 모든 페이지에서 API 호출이 등장합니다. 목록을 불러오고, 상세 정보를 가져오고, 저장 버튼을 누르면 서버에 요청을 보내야 합니다. 그런데 API 호출 코드를 컴포넌트마다 직접 작성하면 `loading`, `error`, `data` 상태가 반복되고, 실패 처리 방식도 화면마다 달라지기 쉽습니다.

`useFetch`는 이런 반복을 줄이는 대표적인 Composable 예제입니다. 핵심은 fetch 자체가 아니라 요청의 상태를 명확하게 나누는 것입니다. 사용자는 데이터를 기다리는 중인지, 성공해서 화면을 그려도 되는지, 실패해서 메시지를 보여줘야 하는지 알아야 합니다.

입문 단계에서는 성공 상황만 예제로 만드는 경우가 많지만, 실무에서는 실패 상황을 먼저 생각하는 편이 안전합니다. 네트워크가 끊길 수도 있고, 서버가 500을 줄 수도 있고, 사용자가 페이지를 빠르게 이동해서 이전 요청 결과가 뒤늦게 도착할 수도 있습니다. 이번 글에서는 그중 가장 기본인 loading, data, error 구조를 잡아보겠습니다.
...
아래 예제는 URL을 받아 데이터를 가져오는 `useFetch`입니다. 컴포넌트에서는 반환된 상태만 사용하고, 요청을 다시 보내고 싶을 때는 `execute`를 호출합니다.
...
<template>
  <p v-if="loading">사용자 목록을 불러오는 중입니다.</p>
  <p v-else-if="error">목록을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.</p>
  <ul v-else>
...
`execute`가 시작되면 `loading`을 true로 바꾸고 이전 error를 비웁니다. 응답이 성공하면 JSON을 data에 넣고, 실패하면 error에 저장합니다. 마지막에는 성공이든 실패든 `finally`에서 loading을 false로 되돌립니다. 이렇게 상태 전환을 한곳에 모아두면 컴포넌트는 화면 조건만 읽으면 되므로 훨씬 이해하기 쉬워집니다.
...
- **확인 포인트**: `fetch`는 HTTP 404나 500을 자동으로 catch로 보내지 않습니다. `response.ok`를 직접 확인하지 않으면 실패 응답을 성공처럼 처리할 수 있습니다. 입문 예제에서는 작게 보일 수 있지만, 실제 프로젝트에서는 원인을 찾기 어려운 버그나 사용자 경험 문제로 이어질 수 있습니다. 코드를 작성한 뒤에는 이 문제가 어떤 상황에서 나타날 수 있는지 직접 한 번 재현해보는 습관을 들이면 좋습니다.
- **확인 포인트**: 에러 메시지를 그대로 사용자에게 보여주면 개발자용 문구가 노출될 수 있습니다. 화면에는 친절한 문구를 보여주고, 실제 에러 객체는 로깅이나 디버깅에 사용하는 편이 좋습니다. 입문 예제에서는 작게 보일 수 있지만, 실제 프로젝트에서는 원인을 찾기 어려운 버그나 사용자 경험 문제로 이어질 수 있습니다. 코드를 작성한 뒤에는 이 문제가 어떤 상황에서 나타날 수 있는지 직접 한 번 재현해보는 습관을 들이면 좋습니다.
- **확인 포인트**: URL이 바뀔 때 자동으로 다시 요청해야 한다면 단순 문자열이 아니라 ref나 getter를 받아 watch하는 구조가 필요합니다. 이 주제는 다음 글들에서 이어집니다. 입문 예제에서는 작게 보일 수 있지만, 실제 프로젝트에서는 원인을 찾기 어려운 버그나 사용자 경험 문제로 이어질 수 있습니다. 코드를 작성한 뒤에는 이 문제가 어떤 상황에서 나타날 수 있는지 직접 한 번 재현해보는 습관을 들이면 좋습니다.
...
공통 `useFetch`를 만들면 반복은 줄어들지만, 모든 API가 같은 방식으로 동작한다고 가정하면 곤란합니다. 인증 헤더, 파일 업로드, 페이지네이션, 캐시 정책이 들어오면 더 구체적인 Composable이나 API 클라이언트 계층으로 나누는 편이 좋습니다.
...
이번 글에서는 'Vue 입문 63: useFetch를 만들며 로딩, 성공, 실패 상태 다루기' 주제를 중심으로 Vue 프로젝트에서 자주 만나는 구조 문제를 살펴봤습니다. 핵심은 코드를 예쁘게 나누는 것이 아니라, 상태와 책임이 어디에 있는지 설명할 수 있게 만드는 것입니다. 예제 코드를 그대로 따라 작성한 뒤에는 이름을 바꿔보거나, 실패 상황을 일부러 만들어보거나, 조건을 하나 추가해보면서 흐름을 직접 확인해보시면 좋습니다.
