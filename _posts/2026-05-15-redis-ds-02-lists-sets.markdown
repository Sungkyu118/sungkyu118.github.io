...
List와 Set은 설명만 보면 단순한 자료구조처럼 느껴지지만, 실제로는 큐 처리와 중복 제거, 이벤트 흐름 설계에서 꽤 다른 성격을 드러냅니다. 그래서 [Redis for 큐: List로 시작하고 Streams를 고민하는 기준](/redis/queue), [Redis Pub/Sub vs Streams: 이벤트 전달을 어디에 써야 할까](/redis/pubsub-vs-streams), [Redis Streams 실전: 작업 큐로 쓰는 법](/redis/streams-queue) 글도 같이 보시면 좋겠습니다. 자료구조 개념을 먼저 익힌 뒤 실제 큐 설계 글까지 이어서 보시면, "왜 어떤 경우에는 List로 충분하고, 왜 어떤 경우에는 Streams까지 가야 하는지"를 훨씬 덜 헷갈리게 이해하실 수 있습니다.
...
- [Redis for 큐: List로 시작하고 Streams를 고민하는 기준](/redis/queue)
  ...
  List를 큐처럼 쓰는 방식은 이 자료구조 글의 가장 직접적인 실전 예시입니다. 단순히 왼쪽에 넣고 오른쪽에서 꺼낸다는 명령어 수준을 넘어, 작업이 실패했을 때 다시 처리할 수 있는지와 처리 순서를 어디까지 보장할 수 있는지까지 생각해보는 데 도움이 됩니다.

- [Redis Pub/Sub vs Streams: 이벤트 전달을 어디에 써야 할까](/redis/pubsub-vs-streams)

  List와 Set을 이해한 뒤 Pub/Sub와 Streams를 비교하면 Redis의 메시지 처리 선택지가 훨씬 입체적으로 보입니다. 순서, 중복, 보관 여부 같은 기준이 자료구조에서 출발해 이벤트 전달 방식으로 확장되는 흐름을 따라갈 수 있습니다.

- [Redis Streams 실전: 작업 큐로 쓰는 법](/redis/streams-queue)

  Streams 글은 List 기반 큐에서 한 단계 더 나아가 소비자 그룹과 재처리까지 고민하는 흐름을 보여줍니다. 자료구조의 기본 감각을 먼저 잡은 뒤 Streams를 보면, 왜 단순 큐로 충분한 상황과 더 정교한 큐가 필요한 상황을 나누어야 하는지 이해하기 쉬워집니다.
